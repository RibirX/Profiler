...
pub const SERVER_ADDR: &'static str = "ws://localhost:31813/socket";

struct Socket(WebSocket<MaybeTlsStream<TcpStream>>);

impl Drop for Socket {
  fn drop(&mut self) { self.0.close(None); }
}
...
pub struct WSHandle {
  addr: String,
  socket: Atomic<Option<Socket>>,
}

fn update_when<T>(ptr: &Atomic<T>, mut check: impl FnMut(&Shared<T>) -> bool, mut val_fn: impl FnMut() -> T) {
...
impl WSHandle {
  pub fn new(addr: String) -> Self { Self { addr, socket: Atomic::default() } }
  pub fn block_connect(&mut self) -> tungstenite::Result<()>{
    ...
    update_when(
      ...
      || connect(Url::parse(&self.addr).unwrap())
          .map_or_else(|e| {
            *res_ref = Err(e);
            None
      ...
      return vals
        ...
        .try_for_each(|msg| {
          socket
            ...
            .or_else(|e| {
              // double calll handle_err to deal with SendQueueFull which may failed again.
...
pub fn add_remote_listener(
  ...
  let handle = consumers.add(Box::new(move |vals| ws2.send_to_remote(vals).unwrap()));
  (ws, handle)
}
...
mod test {
  ...
  fn websocket_log() {
    ...
    let mut consumer = WSHandle {
      ...
      socket: Atomic::new(None),
    };
    consumer.block_connect();
